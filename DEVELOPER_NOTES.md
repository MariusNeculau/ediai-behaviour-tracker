...
- [x] Generare reală rapoarte PDF: raport "management" Individual Child (ReportLab, pure-Python) — Student Details, Key Stats, Incident Summary, Pattern Analysis agregat pe 3 coloane (Triggers/Behaviors/Actions Taken), subsol "Page X of Y", antet cu School Name + Roll Number din SystemConfig. `GET /api/reports/child/<id>?period=week|month|term`, download on-the-fly. Include și Class Summary (`/api/reports/class/<room_id>`, breakdown per-elev) și Whole School (`/api/reports/school`, breakdown per-clasă), prin `render_report_pdf` generalizat.
